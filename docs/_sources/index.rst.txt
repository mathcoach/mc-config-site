.. mc-config-tool documentation master file, created by
   sphinx-quickstart on Sat Jul 30 21:29:00 2022.
   You can adapt this file completely to your liking, but it should at least
   contain the root `toctree` directive.

``mc-config-tool``'s documentation
======================================
...
A library to simplify configuration of Java Software


Quickstart in three steps
-------------------------
In this section we walk through a common situation which happens when one write a java app. We assume, that
the maven is used for build and for dependency management.


Your app needs information about the database URL, the database user and the database password.
These information should not be exposed on any repository. So you want to put these information in a local directory
...
``${HOME}`` is an annotation for the home-directory of the user, who starts the Java-process. It is resolved in Linux
mostly to ``/home/userid`` and in Windows mostly to ``C:\\Users\\userid``.

If your app is distributed, and runs somewhere else, so your users can create a configuration file
...
.. code-block:: xml
   ...
   <properties>
       <mc.config.verion>3.0.3</mc.config.version>
   </properties>
...
Create a Class to *manage* the single instance of ``EnvConfiguration``:
...
If a class needs to *consume* a configuration parameter, it can use one of these pattern:

**Consume an EnvConfiguration**
...
This pattern can be used when a method need many information. In this case, it is not comfortable to put them to
...
The class ``DbConnector`` in this pattern is also testable, the use of method ``DbConnector.getDataSource`` is not
very comfortable. One must remember the order of arugments, here there is only 3 arguments. For more than 3, one
can use the first pattern.
...
database does not exist, ect. cannot be tested in unittest. (At least without changine the configuration)

This pattern is only suitable in the initial code of a app, for example in the ``main(String[] argv)`` or
